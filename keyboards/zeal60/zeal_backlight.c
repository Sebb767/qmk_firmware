...
zeal_backlight_config g_config = {
	...
	.brightness = 255,
	.effect = 2, //255, // Default to RGB test, so Zeal can flash and test in one pass!
	.color_1 = { .h = 0, .s = 0, .v = 255 },
	.color_2 = { .h = 191, .s = 255, .v = 255 },
...
{
	RGB rgb1 = hsv_to_rgb( (HSV){ .h = g_config.color_1.h, .s = g_config.color_1.s, .v = g_config.brightness / 3 } );
...
void backlight_effect_arrow_highlight(void)
{
	RGB color = (RGB){ .r = 255, .g = 0, .b = 255 };
	int mods[] = { 1, 2, 3, 6, 7, 8, 9 };

	uint8_t index;
	for ( int i = 0; i < (sizeof(mods)/sizeof(int)); i++ ) // asdhjkl
	{
		map_row_column_to_led( 2, mods[i], &index );
		backlight_set_color( index, color.r, color.g, color.b );
	}

	map_row_column_to_led( 1, 2, &index ); // w
	backlight_set_color( index, color.r, color.g, color.b );
}
...
ISR(TIMER3_COMPA_vect)
{
	...
	if( IS_LAYER_ON(2) ) {
		backlight_effect_arrow_highlight();
	}
